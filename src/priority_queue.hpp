...
private:
	...
	// Merge two leftist heaps - may throw exception from Compare
	Node* mergeNodes(Node *h1, Node *h2) {
		if (!h1) return h2;
		if (!h2) return h1;

		// Ensure h1 has the larger element (max heap)
		...
		// Maintain leftist property: left child should have >= npl than right
